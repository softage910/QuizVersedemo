...
import { ref, get, set, query } from "firebase/database"; // Import Firebase functions
...
const OnlineTest = () => {
    ...
    const handleNextQuestion = async () => {
        ...
        if (selectedOption) {
            // Save to Firebase Realtime Database
                  ...
                  set(ref(database, `responses/${autoGeneratedId}/day2/${currentQuestionIndex}`), {
            ...
            set(ref(database, `responses/${autoGeneratedId}/day2/${currentQuestionIndex}`), matchedPairs)
  ...
  console.error("Error fetching user data:", error);
    ...
    //         const responsesRef = ref(database, `responses/${autoGeneratedId}/day2`);
    //         const snapshotData = await get(responsesRef);
    ...
    const finishQuiz = async () => {
        try {
            ...
            if (!invitedUsers || !userEmail) {
                console.error("Required session storage data is missing.");
                return;
            }
            ...
            const snapshot = await get(userQuery);
            if (!snapshot.exists()) {
                console.error("No invited users found in Firebase.");
                return;
            }
            ...
            if (!autoGeneratedId) {
                console.error("User ID not found in invitedUsers.");
                return;
            }
            ...
            if (!snapshotData.exists()) {
                console.error("No responses found for the user.");
                return;
            }
            ...
            const formattedResponses = Object.keys(data).map((key) => {
                ...
                if (Array.isArray(response)) {
                    return {
                        question: `Match the following (Q${key})`,
                        answer: response.map(pair => `${pair.left} → ${pair.right}`).join(", "),
                    };
                }
                return {
                    question: response.question || `Q${key}`,
            ...
            const csvData = {
                name: username,
                email: useremail,
                Day: "Day 2 - Online Test – MacOS Fundamentals",
                responses: formattedResponses,
            };
