...
export default function FirstAssessment() {
    ...
    const slides = [
        ...
        {
            image: "Images/leavescreen.png", // replace with your actual image name
            title: "Stay on Screen",
            description:"You must stay on the quiz screen at all times. If you're away from the screen for more than 10 seconds, your attempt will be automatically submitted.",
        }
    ];
    ...
    useEffect(() => {
        ...
        const fetchQuizStatus = async () => {
            ...
            if (!invitedUsers || !UserEmail) {
                console.error("Error: No invited users or user email found.");
                return;
            }
        
            // const parsedInvitedUsers = invitedUsers; // Parse stored string
            ...
            try {
                ...
                if (snapshot.exists()) {
                    ...
                    const autoGeneratedId = Object.keys(userData).find(
                        (key) => userData[key]?.email === UserEmail // Optional chaining to prevent errors
                    );
        
                    if (!autoGeneratedId) {
                        console.error("Error: User not found in invitedUsers.");
                        return;
                    }
        
                    const statusRef = ref(database, `invitedUsers/${autoGeneratedId}/progress/Day2/Assessment`);
                    const statusSnapshot = await get(statusRef);
        
                    if (statusSnapshot.exists()) {
                        setQuizAttempted(statusSnapshot.val());
                    }
                }
            } catch (error) {
